...
void UCSSessionSubsystem::FindSessions(int32 MaxSearchResults, bool IsLANQuery)
{
	...
	if (!sessionInterface.IsValid())
	{
		UE_LOG(LogTemp, Warning, TEXT("Subsystem: Session interface is not valid"));
		OnFindSessionsCompleteEvent.Broadcast(TArray<FOnlineSessionSearchResult>(), false);
		return;
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("Subsystem: Session interface is valid"));
	}
...
bool UCSSessionSubsystem::TryTravelToCurrentSession()
{
	...
	if (!sessionInterface.IsValid())
	{
		UE_LOG(LogTemp, Warning, TEXT("CSSessionSubsystem: Failed to get session interface"));
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("CSSessionSubsystem: Failed to get resolved connect string"));
		return false;
	}
	UE_LOG(LogTemp, Warning, TEXT("CSSessionSubsystem: Connecting with string %s"), *connectString);
	UE_LOG(LogTemp, Warning, TEXT("CSSessionSubsystem: Travelling client"));
	APlayerController* playerController = GetWorld()->GetFirstPlayerController();
	playerController->ClientTravel(FString("192.168.1.41:7777"), TRAVEL_Absolute);
	return true;
}